...
saver = util.SaveHelper(__file__, output_path, suffix=cfg.suffix, cfg=cfg)
...
startup_paths, density_map = stroke_init.init_path_tsp(
    input_img, num_points, startup_w=cfg.startup_width, saliency_type="ood"
)
    ...
    if cfg.b_spline:
        ...
        if cfg.cardinal:
            ...
            path = diffvg_utils.Path(
                Q[:, :2], degree=3, stroke_width=(Q[:, 2], True), closed=cfg.closed
            )
    scene.add_shapes(
        [path], stroke_color=([cfg.alpha], True), fill_color=None, split_primitives=True
    )
...
opt = diffvg_utils.SceneOptimizer(
    scene, params=params, num_steps=cfg.num_opt_steps, lr_min_scale=0.2
)
...
if cfg.sds:
    ...
    opt.add_loss("sds", sds_loss, cfg.sds_w, ("im", "step"))
...
opt.add_loss("style", style_loss, cfg.style_w, ("im",))
...
def frame(step):
    ...
    if must_show:
        ...
        if opt.has_loss("sds"):
            plt.title(
                "Step %d, t %d, %s, %.2f" % (step, int(sds.t_saved), lrs, time_count)
            )
        else:
            plt.title("Iter: %d Step %d, %s" % (iteration, step, lrs))
        opt.plot(50)
