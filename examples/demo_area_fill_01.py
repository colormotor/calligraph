...
saver = util.SaveHelper(__file__, output_path, cfg=cfg)

if cfg.text:
    input_img, outline = plut.font_to_image(
        cfg.text,
...
startup_paths, _ = stroke_init.init_path_tsp(
    img, num_points, closed=cfg.closed, startup_w=cfg.sw
)
    ...
    if cfg.b_spline:
        ...
        if cfg.catmull:
            path = diffvg_utils.CardinalSpline(
                Pw[:, :2], stroke_width=(Pw[:, 2], True), closed=cfg.closed
            )
        else:
            Q = diffvg_utils.cardinal_spline(
                Pw, 0.5, closed=cfg.closed
            )
            if cfg.closed:
                Q = Q[:-1]
...
schedulers = [
    util.step_cosine_lr_scheduler(opt, 0.0, 0.5, cfg.num_opt_steps)
...
losses.add("mse", mse, cfg.mse_w)

if cfg.degree > 3 and cfg.b_spline:
    losses.add("deriv", spline_losses.make_deriv_loss(cfg.deriv, w), cfg.smoothing_w)

losses.add("bbox", spline_losses.make_bbox_loss(geom.make_rect(0, 0, w, h)), 1.0)
...
gs_sub = GridSpecFromSubplotSpec(
    1, 3, width_ratios=[0.15, 0.15, 0.9], subplot_spec=gs[1, :]
)
...
def frame(step):
    ...
    if must_show:
        ...
        plt.imshow(
            target_img * amt + first_frame * (1 - amt), cmap="gray", vmin=0.0, vmax=1.0
        )
        ...
        if losses.has_loss("curv"):
            plut.fill_circle(
                [cfg.target_radius, cfg.target_radius], cfg.target_radius, "c"
            )
        # step_skip = 1
...
plut.show_animation(
    fig, frame, cfg.num_opt_steps, filename=filename, headless=cfg.headless
)
saver.finish()
