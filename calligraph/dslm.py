...
class SLMTrajectory(diffvg_utils.Path):
    ...
    def __init__(self,
        ...
        self.subd = subd
        super().__init__(points, degree=3, closed=False, split_pieces=False,
                         **diffvg_utils.skip_kwargs(['degree', 'closed'], params)) #kwargs)
    ...
    def setup(self):
        ...
        if has_width:
            ...
            Pw = P
        X, dX, ddX, endt, strokes = wslm_kinematics(Pw, Delta_t, Ac, T, delta, m*self.subd, get_strokes=True)
    ...
    def samples(self, subd, get_strokes=False, velocity=False, acceleration=False, delta_t_mul=1.0):
        P = self.param('points')
        m = len(P)-1
        ...
        X, dX, ddX, endt, strokes = wslm_kinematics(P, Delta_t, Ac, T, delta, m*subd, get_strokes=True)
        if get_strokes:
            if velocity:
                return X, dX, strokes
            if acceleration:
    ...
    def samples_with_bounds(self, max_speed, accel_time, dt=0.01, velocity=False): #initial_subd):
        P = self.param('points')
        m = len(P)-1
        Delta_t = self.param('Delta_t')
        ...
        # Hacky dt calc
        num_steps = m*10
        X, dX, ddX, endt, strokes = wslm_kinematics(P, Delta_t, Ac, T, delta, num_steps, get_strokes=True)
    ...
    # def to_dict(self):
    #     d = super().to_dict()
    #     d.update({'Delta_t': self.spline_degree,
...
def lognormal( x, x0, mu, sigma, eps=1e-5):
    ...
    x = x-x0
    x = F.relu(x-eps)+eps
    y = 1 / (((x)*np.sqrt(2*np.pi)*sigma)) * torch.exp(-((torch.log(x)-mu)**2)/(2*sigma**2))
    return y
...
def lognormal_prime(x, x0, mu, sigma):
    ...
    l = 1 / (((x)*np.sqrt(2*np.pi)*sigma)) * torch.exp(-((torch.log(x)-mu)**2)/(2*sigma**2))
...
def mu_sigma(Ac, T):
    ...
    mu = 3. * sigma - tp.log( (-1. + tp.exp(6.*sigma)) / T )
    return mu, sigma
...
def compute_stroke_parameters(P, Delta_t, Ac, T, delta, t_offset=0.):
    m = P.shape[0] - 1
    dim = P.shape[1]
    ...
    t0 = Delta_t[1:] * T[1:]
    t0 = torch.cumsum(torch.cat([torch.ones(1, device=device, dtype=dtype)*t_offset, t0]), dim=0)
...
def wslm_stroke_kinematics(t, t_0, mu, sigma, d, delta):
    ...
    def dR(theta):
        ...
        return torch.stack([
            torch.stack([-st, -ct, torch.zeros_like(ct)], dim=-1),
            torch.stack([ct,  -st, torch.zeros_like(ct)], dim=-1),
            torch.stack([torch.zeros_like(ct), torch.zeros_like(ct), torch.zeros_like(ct)], dim=-1)
        ], dim=1)[:,:dim,:dim]

    H = torch.eye(dim, device=device, dtype=torch.float32)
    H[:2,:2] *= h
    M = (1 / (2*torch.tan(delta / 2))) * torch.tensor([[0, -1, 0], [1, 0, 0], [0, 0, 0]],
                                                      dtype=torch.float32, device=device)[:dim,:dim]
    ...
    if abs(delta) < delta_sensitivity:
        ...
        Md = M@d
        Zd = torch.vstack([0.5*torch.ones_like(w_t),
                           0.5*torch.ones_like(w_t),
                           w_t - 0.5])[:dim,:] * d
...
def wslm_kinematics_raw(num_steps,
                        x0, t0, D, delta, mu, sigma,
                        full_output=False,
                        get_weights=False,
                        t_offset=0.0,
                        get_strokes=False,
    ...
    if dt > 0:
        ...
        t = torch.linspace(start_t, endt, num_steps).to(device) #*endt #, requires_grad=True)
        dt = endt/(num_steps-1)
    ...
    strokes = []
    for i in range(m):
        d, vel, acc = wslm_stroke_kinematics(t, t0[i], mu[i], sigma[i], D[i,:], delta[i])
        X += d
        dX += vel #*dt
        ddX += acc
...
def wslm_kinematics( P,
    ...
    t0, D, delta, mu, sigma = compute_stroke_parameters(P, Delta_t, Ac, T, delta, t_offset)
    return wslm_kinematics_raw(num_steps, P[0], t0, D, delta, mu, sigma,
                               full_output, get_weights, t_offset, get_strokes)
...
def plot_action_plan(Vp, Theta, label='Virtual targets', arc_label='Arcs', markersize=1, clr='r', linewidth=1):
    import matplotlib.pyplot as plt
    subd = 100
    Vp = Vp.T
    m = Vp.shape[1]-1
    ...
    for a, b, theta in zip(Vp.T, Vp[:,1:].T, Theta):
        ...
        A = np.linspace(theta_start-theta, theta_start+theta, subd)
        arc = np.tile(cenp.reshape(-1,1) ,(1,subd)) + np.vstack([np.cos(A), np.sin(A)]) * r
        plt.plot(arc[0,:], arc[1,:], ':'+clr, alpha=0.6, label=lbl, linewidth=linewidth)
        lbl = ''
...
def compute_stroke_parameters_angle(P, Delta_t, Ac, T, delta, t_offset=0.):
    m = P.shape[0] - 1
    dim = P.shape[1]
...
def wslm_stroke_kinematics_angle(t, t_0, mu, sigma, D, theta, delta, gamma=None):
    ...
    if abs(delta) < delta_sensitivity:
        if threeD:
            d = torch.vstack([D*h*torch.cos(theta)*w_t,
                                D*h*torch.sin(theta)*w_t,
                                gamma*w_t]).T
        else:
            d = D*torch.vstack([h*torch.cos(theta)*w_t,
                                h*torch.sin(theta)*w_t]).T
    else:
        if threeD:
            d = torch.vstack([D*h*(torch.cos(theta_0 + w_t*delta) - torch.cos(theta_0))/delta,
                                D*h*(torch.sin(theta_0 + w_t*delta) - torch.sin(theta_0))/delta,
                                gamma*w_t]).T #<- hack
        else:
            d = D*torch.vstack([h*(torch.cos(theta_0 + w_t*delta) - torch.cos(theta_0))/delta,
                                h*(torch.sin(theta_0 + w_t*delta) - torch.sin(theta_0))/delta]).T #<- hack
    # Velocity
    # Assume 2.5 d
    if threeD:
        vel = torch.vstack([D * l * torch.cos(phi),
                            D * l * torch.sin(phi),
                            l * gamma]).T
    else:
        vel = torch.vstack([D * l * torch.cos(phi),
                            D * l * torch.sin(phi)]).T
    # Acc (invalid if 3d)
    acc = torch.vstack([D*dl*torch.cos(phi) - D*delta*(l**2)*torch.sin(phi),
                        D*dl*torch.sin(phi) + D*delta*(l**2)*torch.cos(phi)]).T

    return d, vel, acc

def wslm_kinematics_angle( P,
                      Delta_t,
                      Ac,
                      T,
                      delta,
    ...
    ''' Weighted parameterisation of a sigma-lognormal trajectory
        using alpha ]0,1] as an assymetry parameter,
        and d for the duration of the lognormal '''
    t0, D, theta, delta, mu, sigma, gamma = compute_stroke_parameters(P, Delta_t, Ac, T, delta, t_offset)
    # endtime
    endt = t0[-1] + torch.exp( mu[-1] + sigma[-1]*3 )
    # time steps
